...
NEGATE = [t.strip() for t in open(os.path.join(PATH, 'lexicons/negate.txt'))]

# Booster/dampener 'intensifiers' or 'degree adverbs' (Portuguese)
boosters = []
for l in open(os.path.join(PATH, 'lexicons/booster.txt')):
    parts = l.strip().split(' ')
...
class SentimentIntensityAnalyzer(object):
    ...
    def __init__(self, lexicon_file="lexicons/vader_lexicon_ptbr.txt", emoji_lexicon="lexicons/emoji_utf8_lexicon_ptbr.txt"):
        ...
        with open(os.path.join(PATH, lexicon_file), encoding='utf-8') as f:
            self.lexicon_full_filepath = f.read()
        self.lexicon = self.make_lex_dict()

        with open(os.path.join(PATH, emoji_lexicon), encoding='utf-8') as f:
